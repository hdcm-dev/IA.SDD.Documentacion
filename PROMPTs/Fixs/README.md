...
| `03` | [Fix-Analizar-Reporte-25.md](03-Fix-Reporte-25/Fix-Analizar-Reporte-25.md) | `25` — el producto evoluciona y el método no tiene dónde registrarlo | **Aplicado.** Cerró en **SDD 13.14**, el 2026-09-12, **cuarta y última del orden `04` → `06` → `05` → `03`**. La pregunta de fondo —¿corresponde un tercer caso de escritura del intake?— se resolvió que **no**, precedente el `12`: los dos casos de `Master-Prompt.md` §13 ocurren durante una corrida del orquestador y una decisión de alcance posterior al handoff no, y el intake ya era documento humano editable fuera de una corrida. Lo que faltaba era el evento y el criterio que reabren backlog y roadmap, no un permiso: entraron en `Rules-Backlog-Tecnico.md` §3.6 y `Rules-Contexto.md` §3.5. El paso 6 de §7 del reporte queda sin re-ejecutar sobre `Lab-Geometria`, declarado en su [`OUTPUTs/30`](03-Fix-Reporte-25/OUTPUTs/30-Plan-Y-Verificacion-Criterios.md) |
...
| `07` | [Fix-Analizar-Reporte-29.md](07-Fix-Reporte-29/Fix-Analizar-Reporte-29.md) | `29` — el umbral de archivos individuales no dice de qué se cuenta | **Aplicado.** Cerró en **SDD 13.15**, el 2026-09-12. La decisión de fondo —de qué se cuenta el umbral, §5.1— se resolvió **por proyecto de código**, no por conteo de menciones (cuatro contra dos) sino por impacto medido: bajo esa lectura ningún destino del workspace incumple, y bajo «unidad de entrega» dos destinos reales incumplirían de golpe en cuatro documentos. La solicitud 6 quedó resuelta en el mismo texto, porque §3.3, §6 y §8 tratan US y BT en los mismos párrafos. El caso mixto de un documento consolidado con proyectos a los dos lados del umbral queda declarado **forma correcta y no transitoria**, verificado en vivo contra `Lab-Geometria` mientras esa fase seguía en curso. Los cinco criterios de §7 quedan cumplidos, declarados en su [`OUTPUTs/30`](07-Fix-Reporte-29/OUTPUTs/30-Verificacion-Del-Plan-Aplicado.md) |
| `08` | [Fix-Analizar-Reporte-30.md](08-Fix-Reporte-30/Fix-Analizar-Reporte-30.md) | `30` — el método no sabe poner un proyecto de otro ecosistema en la solución | **Aplicado.** Cerró en **SDD 13.16**, el 2026-09-13. La pregunta de fondo se resolvió que **sí**: un proyecto de otro ecosistema pertenece a la solución de código de un proyecto que toma su artefacto como **insumo de construcción** —la segunda clase de arista del grafo, con único generador—, porque la solución se delimita por su comando de construcción. Entraron la cadena de herramientas ausente y el sample en el agrupador con su verificación fuera de la construcción, §16.1 dice D8 de la unidad, y **ninguna herramienta entró a una regla**: el único ecosistema nombrado es el ejemplo del perfil de convención del manifiesto. El nombre del destino no se adoptó por colisión medida, y P-8 de la especificación quedó superada en su premisa. Los cinco criterios de §7, cumplidos, declarados en su [`OUTPUTs/30`](08-Fix-Reporte-30/OUTPUTs/30-Verificacion-Del-Plan-Aplicado.md) |
| `09` | [Fix-Analizar-Reporte-31.md](09-Fix-Reporte-31/Fix-Analizar-Reporte-31.md) | `31`: el método no expedienta sus casos | **Aplicado.** Cerró en **SDD 13.18**, el 2026-09-13. La pregunta de fondo se resolvió que **sí**: el caso entra como figura normada en `Expediente-Rules.md`, con una forma mínima de **diecinueve** campos que se llena en minutos y **doce criterios con su comando en la regla, cada uno probado fallando**. El dictamen del expediente `0001` no se aplicó al pie de la letra: lo atacaron siete comisiones, entre ellas Seguridad, Formal y Trazabilidad, y lo votó **un jurado de cinco agentes distintos**, diecisiete ítems 5-0. **D9 no se modifica** (E-3 por la opción C). En la misma versión se ofuscó `Examples/` y se redactó por S2 el expediente `0001`, que queda como forma histórica. Cuatro criterios de §7 cumplidos y uno sin veredicto hasta una migración real, declarados en su [`OUTPUTs/30`](09-Fix-Reporte-31/OUTPUTs/30-Verificacion-Del-Plan-Aplicado.md) |
